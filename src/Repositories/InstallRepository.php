...
namespace Marifuli\Service\Repositories;
...
class InstallRepository
{
    ...
    public function checkReinstall()
    {
        try {
            ...
            return (Storage::exists('.install_count') ? Storage::get('.install_count') : 0) and (Artisan::call('marifuli:migrate-status'));
        } catch (Exception $e) {
            return false;
        }
    }
    ...
    public function getPreRequisite()
    {
        ...
        $server[] = $this->check($this->my_version_compare(phpversion(), config('marifuli.php_version', '7.2.0'), '>='), sprintf('Min PHP version ' . config('marifuli.php_version', '7.2.0') . ' (%s)', 'Current Version ' . phpversion()), 'Current Version ' . phpversion(), true);
        ...
        $verifier = verifyUrl(config('marifuli.verifier', 'auth'));
    ...
    public function validateLicense($params)
    {
        ...
        $url = verifyUrl(config('marifuli.verifier', 'auth')) . '/api/cc?a=install&u=' . app_url() . '&ac=' . request('access_code') . '&i=' . config('app.item') . '&e=' . request('envato_email') . '&ri=' . request('re_install') . '&current=' . urlencode(request()->path()) . '&ve=' . $ve . '&v=' . $v;
        ...
        if ($status) {
            $checksum = $response['checksum'] ?? '234sadfawfawe';
            $license_code = $response['license_code'] ?? 'awefawefwafwefwa';
    ...
    public function checkLicense()
    {
        ...
        // $url = verifyUrl(config('marifuli.verifier', 'auth')) . '/api/cc?a=verify&u=' . app_url() . '&ac=' . $ac . '&i=' . config('app.item') . '&e=' . $e . '&c=' . $c . '&v=' . $v . '&current=' . urlencode(request()->path());
        // $response = curlIt($url);
        // if ($goto = gv($response, 'goto')) {
        //     return redirect($goto)->send();
        // }
        $status = true; //gbv($response, 'status');
    ...
    {
        $db_connection = config('marifuli.support_multi_connection', false) ? gv($params, 'db_connection', 'mysql') : 'mysql';
        envu([
            'APP_URL' => app_url(),
    ...
    public function migrateDB()
    {
        ...
        try {
            ...
            Log::error($e);
            return false;
            $sql = base_path('database/' . config('marifuli.database_file'));
            if (File::exists($sql)) {
    ...
    public function processModuleInstallation($params, $response)
    {
        ...
        if (!$row) {
            if ($file) {
                ...
                if (!Schema::hasColumn(config('marifuli.settings_table'), $name)) {
                    Schema::table(config('marifuli.settings_table'), function ($table) use ($name) {
                        $table->integer($name)->default(1)->nullable();
                    });
                }
            }
        } else {
            $settings_model_name = config('marifuli.settings_model');
        ...
        try {
            ...
            DB::beginTransaction();
            $module_class_name = config('marifuli.module_manager_model');
    ...
    protected function disableModule($module_name, $row = false, $file = false)
    {
        ...
        $module_model_name = config('marifuli.module_model');
    ...
    public function installTheme($params)
    {
        ...
        $query = DB::table(config('marifuli.theme_table', 'themes'))->where('name', $name);
        $theme = $query->first();
        ...
        $url = verifyUrl(config('marifuli.verifier', 'auth')) . '/api/cc?a=install&u=' . app_url() . '&ac=' . $code . '&i=' . $item_id . '&e=' . $e . '&t=Theme';
