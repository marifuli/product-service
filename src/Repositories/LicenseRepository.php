...
class LicenseRepository
{
    ...
    public function revoke()
    {
        ...
        $url = verifyUrl(config('marifuli.verifier', 'auth')) . '/api/cc?a=remove&u=' . app_url() . '&ac=' . $ac . '&i=' . config('app.item') . '&e=' . $e . '&c=' . $c . '&v=' . $v;
    ...
    public function revokeModule($params)
    {
        ...
        if ($s) {
            ...
            $url = verifyUrl(config('marifuli.verifier', 'auth')) . '/api/cc?a=remove&u=' . app_url() . '&ac=' . $s->purchase_code . '&i=' . $item_id . '&t=Module' . '&v=' . $version . '&e=' . $e;
    ...
    public function revokeTheme($params)
    {
        ...
        if ($s) {
            ...
            $url = verifyUrl(config('marifuli.verifier', 'auth')) . '/api/cc?a=remove&u=' . app_url() . '&ac=' . $s->purchase_code . '&i=' . $s->item_code . '&t=Theme' . '&v=' . $s->version . '&e=' . $s->email;
            ...
            if ($s->is_active == 1) {
                $default = DB::table(config('marifuli.theme_table', 'themes'))->where('id', 1)->update(
                    [
                        'is_active' => 1
                    ]
                );

                $check = DB::table(config('marifuli.theme_table', 'themes'))->where('is_active', 1)->first();
