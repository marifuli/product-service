...
class InitRepository {
    ...
    public function checkDatabase(){
        ...
        try {
            if (!Storage::has('settings.json')) {
                ...
                if (!Schema::hasTable(config('marifuli.settings_table')) || !Schema::hasTable('users')){
                    return false;
                }
            }
        } catch(\Exception $e){
    ...
    public function check() {
        ...
        $url = verifyUrl(config('marifuli.verifier', 'auth')) . '/api/cc?a=verify&u=' . app_url() . '&ac=' . $ac . '&i=' . config('app.item') . '&e=' . $e . '&c=' . $c . '&v=' . $v.'&current='.urlencode(request()->path()).'&ve='.$ve;
        $response = curlIt($url);
    ...
    public function apiCheck(){
        ...
        $ve = Storage::exists('.ve') ? Storage::get('.ve') : 'e';
        $url = verifyUrl(config('marifuli.verifier', 'auth')) . '/api/cc?a=verify&u=' . app_url() . '&ac=' . $ac . '&i=' . config('app.item') . '&e=' . $e . '&c=' . $c . '&v=' . $v.'&ve='.$ve;
        $response = curlIt($url);
    ...
    public function product() {
        ...
        $about = file_get_contents(verifyUrl(config('marifuli.verifier', 'auth')) . '/about');
        $update_tips = file_get_contents(verifyUrl(config('marifuli.verifier', 'auth')) . '/update-tips');
        $support_tips = file_get_contents(verifyUrl(config('marifuli.verifier', 'auth')) . '/support-tips');

        $url = verifyUrl(config('marifuli.verifier', 'auth')) . '/api/cc?a=product&u=' .  app_url() . '&ac=' . $ac . '&i=' . config('app.item') . '&e=' . $e . '&c=' . $c . '&v=' . $v;
